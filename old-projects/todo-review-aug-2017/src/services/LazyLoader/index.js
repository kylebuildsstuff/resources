...
export class LazyLoader extends React.Component {
  ...
  render() {
    const { loader, ...otherProps } = this.props; // Object spread, same concept as Array spreads/rest parameters
    // otherProps is an arbitrary name for the rest of the parameters
    const { AsyncModule } = this.state;

    if (AsyncModule) {
      return (
        <div ref="isMounted">
          <AsyncModule {...otherProps} />
        </div>
      )
    }
