...
const validate = (values, props) => { // eslint-disable-line
  ...
  if (!values.get(schema.removing_vehicle.name)) {
    errors[schema.removing_vehicle.name] = 'Required';
  }

  if (!values.get(schema.removing_vehicle_status.name)) {
    errors[schema.removing_vehicle_status.name] = 'Required';
  }

  if (values.get(schema.removing_vehicle_status.name) && (
    !values.get(schema.removing_vehicle_loss_of_possession_date.name)
  )) {
    errors[schema.removing_vehicle_loss_of_possession_date.name] = 'Required';
  }
  ...
  } else if (values.get(schema.adding_vehicle_year.name) > 1981 && !values.get(schema.adding_vehicle_make.name)) {
    errors[schema.adding_vehicle_make.name] = 'Required';
  }
  ...
  if (values.get(schema.vehicle_condition.name) === VEHICLE_CONDITIONS.new || values.get(schema.vehicle_condition.name) === VEHICLE_CONDITIONS.demo) {
